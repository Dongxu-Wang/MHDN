...
class v8DetectionLoss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        self.contrast_warmup = 0

        self.assigner = TaskAlignedAssigner(topk=10, num_classes=self.nc, alpha=0.5, beta=6.0)
        self.bbox_loss = BboxLoss(m.reg_max - 1, use_dfl=self.use_dfl).to(device)
        self.proj = torch.arange(m.reg_max, dtype=torch.float, device=device)
        self.contrast_loss = PixelContrastLoss(ignore_label=-1, temperature=self.temperature).to(device)
    ...
    def __call__(self, preds, batch):
        ...
        if self.training and batch['with_contrast_loss'] is True:
            proj_head = preds[1]
            proj_head= torch.cat([xi.view(proj_head[0].shape[0], proj_head[0].shape[1], -1) for xi in proj_head], 2).permute(0, 2, 1).contiguous()
            preds = preds[0]
        ...
        # fg_mask为正负样本掩码,false为负样本，true为正样本
        ...
        loss_test = self.bce(pred_scores, target_scores.to(dtype)).sum() / target_scores_sum  # BCE
        ...
        # Contrast loss
        if self.training and batch['with_contrast_loss'] is True:
            loss[3] = self.contrast_loss(proj_head, target_scores)
            loss[3] *= self.hyp.contrast
        ...
        return loss.sum() * batch_size, loss.detach()  # loss(box, cls, dfl, contrast)
    
class PixelContrastLoss(nn.Module):
    def __init__(self, ignore_label, temperature):
    ...
    def _contrastive(self, feats_, labels_):
        # feats_: [batch-size中出现的类别, hard_anchor_sampling后的样本, C]

        labels_ = labels_.contiguous().view(-1, 1)


        ## ------------去除样本中只有一个的样本，避免损失变nan------------
        torch.use_deterministic_algorithms(False, warn_only=False)
        counts = torch.bincount(labels_.squeeze(1))
        m = counts[labels_] >= 2
        feats_ = feats_[m.squeeze(1)]
        labels_ = labels_[m].unsqueeze(1)
        #-----------------------------------------------------------

        mask = torch.eq(labels_, torch.transpose(labels_, 0, 1)).float()     #创建掩码矩阵，标签相同的样本为1，标签不同的样本为0，对角线必为1，目的是找出标签相同的样本

        contrast_count = feats_.shape[0]
        contrast_feature = feats_

        anchor_feature = contrast_feature

        anchor_dot_contrast = torch.div(torch.matmul(anchor_feature, torch.transpose(contrast_feature, 0, 1)),
                                        self.temperature).to(feats_.dtype)   # 计算anchor与contrast的相似度，即logits
        logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True)
        logits = anchor_dot_contrast - logits_max.detach()

        neg_mask = 1 - mask

        # logits_mask = torch.ones_like(mask) - torch.eye(contrast_count, device=mask.device)
        # mask = mask * logits_mask # 将掩码矩阵与理论上的掩码矩阵相乘，得到真实的正样本掩码矩阵

        neg_logits = torch.exp(logits) * neg_mask
        neg_logits = neg_logits.sum(1, keepdim=True) # 将负样本的logits求和，得到每个anchor的负样本logits之和
    ...
    def forward(self, feats, labels=None):
        feats = feats[torch.nonzero(labels)[:, 0], torch.nonzero(labels)[:, 1]]
        cls_label = torch.nonzero(labels)[:, 2]
        loss = self._contrastive(feats, cls_label)
